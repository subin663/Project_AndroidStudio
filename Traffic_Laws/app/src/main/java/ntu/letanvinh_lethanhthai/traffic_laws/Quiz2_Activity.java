...
public class Quiz2_Activity extends AppCompatActivity  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (questionList != null) {
            ...
            Log.e("Quiz2_Activity", "Không thể load danh sách câu hỏi.");
    ...
    private List<All_Question> loadQuestionsFromAssets() {
        ...
        try {
            //Ở đây bạn đang tải quiz_2.json
            InputStream is = getAssets().open("quiz_2.json");
            int size = is.available();
            Log.d("DEBUG", "Size of quiz_2.json: " + size); // Đổi tên file trong log
            byte[] buffer = new byte[size];
            int read = is.read(buffer);
            Log.d("DEBUG", "Bytes read from quiz_2.json: " + read); // Đổi tên file trong log
            is.close();
            ...
            Log.e("Quiz2_Activity", "Error loading questions: ", e); // Đổi tên Activity trong log
            e.printStackTrace();
            return null;
        }
        return questions;
